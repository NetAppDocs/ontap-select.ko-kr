...
ONTAP Select는 VMware vSphere 및 KVM(커널 기반 가상 머신)과 같은 클라우드 관리 프레임워크에 통합되어 있습니다. 이를 통해 파일 서비스, 홈 디렉터리, 소프트웨어 개발 환경 및 애플리케이션 테스트와 같은 새로운 프로젝트의 배포 속도를 높일 수 있습니다.
...
ONTAP Select 클러스터는 1개, 2개, 4개, 6개, 8개, 10개 또는 12개의 노드로 구성됩니다. 각 클러스터 노드는 별도의 가상 머신으로 배포되며 특별히 설계된 ONTAP 9 소프트웨어 버전을 실행합니다.
...
== FlexCache ONTAP Select를 사용한 쓰기 저장

ONTAP Select는 쓰기 백 기능을 지원하는 FlexCache 캐시 볼륨을 지원합니다. ONTAP Select 인스턴스의 전반적인 성능은 기본형 ONTAP 시스템과 유사합니다.

ONTAP Select에서 쓰기 후 기능이 있는 FlexCache 캐시 볼륨에는 다음 요구 사항이 있습니다.

* 중간 규모(8코어) 또는 대규모(16코어) 인스턴스에서 ONTAP Select를 사용하십시오.
* NVMe 기반 솔리드 스테이트 드라이브를 사용하십시오. SAS 기반 스토리지는 ONTAP Select의 경우 NVMe 기반 스토리지에 비해 성능이 낮습니다.
* 배포를 시작하기 전에 FlexCache 쓰기 백에 대한 ONTAP Select를 테스트하십시오. FlexCache 쓰기 백은 많은 수의 작은 파일에는 권장되지 않습니다.


ONTAP Select는 소형 인스턴스(4코어)에서 쓰기 지연 기능을 사용하는 FlexCache 캐시 볼륨을 지원하지 않습니다. 또한 ONTAP Select는 소형, 중형 또는 대형 인스턴스에서 쓰기 지연 기능을 사용하는 FlexCache 원본 볼륨을 지원하지 않습니다.

link:https://docs.netapp.com/us-en/ontap/flexcache-writeback/flexcache-write-back-guidelines.html["ONTAP FlexCache 쓰기 백에 대해 알아보세요"^]
...
.관련 정보
* link:reference_lic_ontap_features.html#licensed-ontap-features["기본적으로 라이선스가 부여되는 ONTAP 기능에 대해 알아보십시오"]
* link:concept_lic_evaluation.html["ONTAP Select 라이센스 옵션에 대해 알아봅니다"]
