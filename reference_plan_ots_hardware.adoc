...
NOTE: 당신은 검토해야합니다link:reference_release_notes.html["ONTAP Select 릴리즈 노트"] 추가로 알려진 제한이나 제약 사항이 있는 경우.
...
* Red Hat Enterprise Linux(RHEL) 10.1, 10.0, 9.7, 9.6, 9.5, 9.4, 9.2, 9.1, 9.0, 8.8, 8.7 및 8.6의 KVM
* Rocky Linux 10.1, 10.0, 9.7, 9.6, 9.5, 9.4, 9.3, 9.2, 9.1, 9.0, 8.9, 8.8, 8.7 및 8.6의 KVM
...
NOTE: ESXi 7.0 GA가 더 이상 제공되지 않는 상태에 도달했습니다.  이 버전의 ONTAP Select 클러스터가 있는 경우 지원되는 버전으로 업그레이드해야 합니다. https://mysupport.netapp.com/matrix["상호 운용성 매트릭스 툴(IMT)"^] .
...
ONTAP Select는 Intel Xeon Sandy Bridge 및 Advanced Micro Devices(AMD) 프로세서 기반 CPU 모델을 지원합니다.

자세한 내용은 https://mysupport.netapp.com/matrix["상호 운용성 매트릭스 툴"^]를 참조하십시오.
...
.프로세서
지원되는 마이크로프로세서에는 Intel Xeon 서버 프로세서와 AMD 프로세서가 포함됩니다.

자세한 내용은 link:https://www.intel.com/content/www/us/en/products/processors/xeon/view-all.html?Processor+Type=1003["Intel Xeon 프로세서"^] 및 link:https://www.amd.com/en/products/processors/server/epyc.html["AMD EPYC 서버 CPU"^]을 참조하십시오.
...
| 4, 6, 8, 10 또는 12노드 클러스터 | 10GbE 2개 | 4 x 10GbE 또는 2 x 25/40GbE 
|===
...
.관련 정보
link:concept_lic_platforms.html["ONTAP Select의 플랫폼 라이센스 오퍼링"]에 대해 알아보십시오
