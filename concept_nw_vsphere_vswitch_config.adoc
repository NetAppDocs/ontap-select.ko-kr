...
다중 노드 클러스터의 경우 ONTAP Deploy는 각 ONTAP Select VM이 내부 네트워크에 하나 또는 두 개의 포트 그룹을 사용하고 별도로 외부 네트워크에 하나 또는 두 개의 포트 그룹을 사용하도록 구성합니다. 클러스터 및 노드 관리 트래픽은 외부 트래픽과 동일한 포트 그룹을 사용하거나 선택적으로 별도의 포트 그룹을 사용할 수 있습니다. 클러스터 및 노드 관리 트래픽은 내부 트래픽과 동일한 포트 그룹을 공유할 수 없습니다.
...
다중 노드 클러스터의 각 노드에 4개의 포트 그룹을 할당할 수 있습니다. 각 포트 그룹에는 다음 그림과 같이 하나의 활성 물리적 포트와 3개의 대기 물리적 포트가 있습니다.
...
두 개의 물리적 포트(10Gb 이하)를 사용하는 경우, 각 포트 그룹에는 활성 어댑터와 대기 어댑터가 서로 반대로 구성되어야 합니다. 내부 네트워크는 다중 노드 ONTAP Select 클러스터에만 존재합니다. 단일 노드 클러스터의 경우, 외부 포트 그룹에서 두 어댑터 모두 활성으로 구성할 수 있습니다.

다음 예는 다중 노드 ONTAP Select 클러스터의 내부 및 외부 통신 서비스를 처리하는 vSwitch 및 두 개의 포트 그룹의 구성을 보여줍니다. 내부 네트워크 VMNIC가 이 포트 그룹에 속해 있고 대기 모드로 구성되어 있으므로 네트워크 장애 발생 시 외부 네트워크는 내부 네트워크 VMNIC를 사용할 수 있습니다. 외부 네트워크의 경우는 이와 반대입니다. 두 포트 그룹 간에 활성 및 대기 VMNIC를 번갈아 사용하는 것은 네트워크 장애 발생 시 ONTAP Select VM의 올바른 페일오버를 위해 매우 중요합니다.
