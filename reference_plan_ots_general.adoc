...
필요한 구성 및 소프트웨어 패키지를 포함하여 Linux 서버 준비에 대한 자세한 내용은 link:kvm-host-configuration-and-preparation-checklist.html["호스트 구성 체크리스트"]을(를) 참조하십시오. 현재 지원되는 Linux 배포판의 하이퍼바이저 요구 사항을 참조하십시오.
...
클러스터의 노드 수입니다:: ONTAP Select 클러스터는 1개, 2개, 4개, 6개, 8개, 10개 또는 12개의 노드로 구성됩니다. 애플리케이션 요구 사항에 따라 클러스터 크기를 결정해야 합니다. 예를 들어, 엔터프라이즈 배포에 HA 기능이 필요한 경우 다중 노드 클러스터를 사용해야 합니다.
...
지원되는 플랫폼에 대한 자세한 내용은 하이퍼바이저별 계획 정보 및 link:reference_release_notes.html["릴리스 노트"]를 참조하십시오.
