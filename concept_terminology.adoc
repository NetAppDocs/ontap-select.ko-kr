...
ONTAP Select 클러스터:: 1개, 2개, 4개, 6개, 8개, 10개 또는 12개의 노드로 구성된 _ONTAP Select 클러스터_를 만들 수 있습니다. 다중 노드 클러스터는 항상 하나 이상의 HA 쌍을 포함합니다. 예를 들어, 4노드 클러스터는 2개의 HA 쌍으로 구성됩니다. 단일 노드 클러스터는 HA 기능을 제공하지 않습니다.
...
자세한 내용은 두 섹션 link:concept_workflow_deploy.html["계획"] 및 link:concept_lic_evaluation.html["라이센스"]을 참조하십시오.
...
자격 증명 저장소:: Deploy 자격 증명 저장소는 계정 자격 증명을 보유하는 보안 데이터베이스입니다. 주로 새 클러스터 생성 과정에서 하이퍼바이저 호스트를 등록하는 데 사용됩니다. 자세한 내용은 link:concept_workflow_deploy.html["계획"] 섹션을 참조하십시오.
...
+
--
vNAS 기본 구성에서는 SIDL(단일 인스턴스 데이터 로깅)이라는 쓰기 최적화 기능을 활성화합니다. ONTAP Select 9.6 이상 릴리스에서는 SIDL이 활성화된 상태에서 백그라운드 ONTAP 스토리지 효율성 기능이 검증됩니다. 자세한 내용은 link:concept_stor_concepts_chars.html["딥 다이브"] 섹션을 참조하십시오.
...
ONTAP Select 이미지 설치:: Deploy 관리 유틸리티에는 ONTAP Select 의 단일 버전만 포함되어 있습니다. 포함된 버전은 출시 시점을 기준으로 사용 가능한 최신 버전입니다. ONTAP Select 이미지 설치 기능을 사용하면 Deploy 유틸리티 인스턴스에 이전 버전의 ONTAP Select 추가하여 ONTAP Select 클러스터를 배포할 때 사용할 수 있습니다. 보다link:task_cli_deploy_image_add.html["ONTAP Select 이미지 추가 를 참조하십시오"] .
