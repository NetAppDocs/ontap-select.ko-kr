...
NetApp 라이센스 파일 지원이 업데이트되었습니다:: ONTAP Select 9.16.1부터 NetApp 라이센스 파일(NLF) 지원이 업데이트되었습니다. 새로운 NLF 형식에는 ARP, ONTAP S3 및 S3 SnapMirror 기능에 대한 라이센스가 포함됩니다. link:reference_lic_ontap_features.html#enabled-ontap-features["자세한 정보"].
+
--
...
--
자율적 랜섬웨어 방어 지원:: ONTAP Select 9.16.1은 Autonomous Ransomware Protection(ARP)에 대한 지원을 도입합니다. ONTAP Select 9.16.1은 ARP에 대한 수동 업데이트만 지원하며 자동 업데이트는 지원되지 않습니다. ARP 기능 라이센스는 ONTAP Select 9.16.1용 NLF에 포함되어 있습니다. link:reference_lic_ontap_features.html#enabled-ontap-features["자세한 정보"].
