...
클러스터 확장:: ONTAP Select 9.13.1에서는 클러스터 확장을 도입하여 노드 중단이 데이터를 제공하는 클러스터 기능에 영향을 미침으로부터 고가용성(HA) 보호 기능을 제공합니다. ONTAP Select 9.13.1부터 두 노드를 HA 쌍으로 구성하여 단일 노드 클러스터를 2노드 클러스터로 확장할 수 있습니다. 이 절차는 ONTAP Select Deploy에서 CLI 또는 웹 인터페이스를 사용하여 자동화되고 시작됩니다. 이 절차를 수행하려면 특정 하드웨어 및 소프트웨어 요구 사항을 충족해야 합니다. link:concept_cluster_expansion.html["자세한 정보"].
