...
외부 키 관리(KMIP) 지원:: ONTAP Select 9.19.1부터 ONTAP Select는 KMIP(Key Management Interoperability Protocol)를 사용한 외부 키 관리를 지원합니다. ONTAP Select 9.19.1 클러스터를 배포하거나 기존 클러스터를 ONTAP Select 9.19.1로 업그레이드한 후 외부 키 관리를 활성화할 수 있습니다. 외부 키 관리는 ONTAP Select 9.19.1에서 지원하는 유일한 키 관리입니다. 자세한 내용은 link:reference_lic_ontap_features.html#licensed-ontap-features["기본적으로 라이센스가 부여되는 ONTAP 기능"]를 참조하십시오.
+
--
ONTAP Select는 클라우드 키 관리자 또는 ONTAP 온보드 키 관리자를 지원하지 않습니다.

--
Advanced Micro Devices(AMD) 프로세서 지원:: ONTAP Select 9.19.1부터 ONTAP Select는 KVM(커널 기반 가상 머신) 및 ESXi 하이퍼바이저 호스트에서 AMD 프로세서를 지원합니다. 자세한 내용은 link:reference_plan_ots_hardware.html["VMware 하이퍼바이저 및 하드웨어 고려사항"]를 참조하십시오.
...
ONTAP Select 제품 다운로드 옵션 업데이트:: NetApp Support 사이트의 ONTAP Select 제품 다운로드 옵션이 ONTAP Select 9.18.1 릴리스부터 *ONTAP Select Deploy* 및 *ONTAP Select Image*로 변경됩니다. 다음 표에 나와 있는 것처럼 이 두 가지 새로운 제품 다운로드 옵션이 기존의 네 가지 옵션을 대체합니다.
+
--
[cols="30,30,40"]
|===
| 이전 제품 다운로드 옵션 | 새로운 제품 다운로드 옵션 | 자세한 내용은 다음을 참조하십시오.... 


| ONTAP Select | ONTAP Select 구축 | link:task_install_deploy.html["ONTAP Select Deploy를 설치합니다"] 


| ONTAP Select 이미지 설치 | ONTAP Select 이미지 | link:task_cli_deploy_image_add.html["배포할 ONTAP Select 이미지를 추가합니다"] 


| ONTAP Select Deploy 업그레이드 | ONTAP Select 구축 | link:task_cli_upgrade_deploy.html["ONTAP Select 배포 인스턴스를 업그레이드합니다"] 


| ONTAP Select 노드 업그레이드 | ONTAP Select 이미지 | link:concept_adm_upgrading_nodes.html["ONTAP Select 노드 업그레이드"] 
|===
--
...
--
KVM 하이퍼바이저 지원이 업데이트되었습니다:: ONTAP Select 9.18.1부터 Rocky Linux 10.1에서 KVM 하이퍼바이저가 지원됩니다.
...
KVM 호스트의 로컬 연결 NVMe 디스크에 대한 소프트웨어 RAID 지원:: ONTAP Select 9.17.1에서는 KVM 하이퍼바이저 호스트에 로컬로 연결된 NVMe 디스크에 대한 소프트웨어 RAID 지원이 도입되었습니다.
+
--
...
--
SnapMirror 클라우드 지원:: ONTAP Select 9.17.1부터 SnapMirror 클라우드가 ONTAP Select에서 지원됩니다. SnapMirror 클라우드는 기본적으로 ONTAP Select에 대해 라이센스가 부여되지 않습니다. ONTAP Select 9.17.1 클러스터를 배포하거나 기존 ONTAP Select 클러스터를 버전 9.17.1로 업그레이드할 때 SnapMirror 클라우드 라이센스를 요청해야 합니다. 자세한 내용은 link:reference_lic_ontap_features.html#other-ontap-features["ONTAP 기능은 별도로 라이센스가 부여됩니다"]을 참조하십시오.
SnapLock Select 지원:: ONTAP Select 9.17.1 버전부터는 신규 구축 시 SnapLock Select(위변조 방지 스냅샷 잠금 기능 포함) 라이선스가 자동으로 적용됩니다. ONTAP Select 9.16.1 이하 버전에서 업그레이드하는 경우, SnapLock Select 라이선스를 link:https://mysupport.netapp.com/site/["NetApp Support 사이트"^]에서 무료로 다운로드하여 수동으로 적용할 수 있습니다. 자세한 내용은 link:reference_lic_ontap_features.html#licensed-ontap-features["기본적으로 라이센스가 부여되는 ONTAP 기능"]을 참조하십시오.
...
+
--
vSAN ESA는 ESXi 전용 구성이며 vSphere 8에서 도입된 VMware vSAN의 새로운 아키텍처입니다. vSAN ESA는 특히 NVMe 기반 TLC 플래시 장치를 사용할 때 향상된 효율성, 확장성 및 성능을 제공하도록 설계되었습니다.

ONTAP Select Deploy 및 ONTAP Select for ESXi는 스토리지 풀에 vSAN 또는 외부 어레이 유형의 데이터 저장소를 사용하여 ONTAP Select 단일 노드 및 다중 노드 클러스터 구성을 지원합니다.
...
--
KVM 하이퍼바이저 지원이 업데이트되었습니다:: ONTAP Select 9.17.1부터 KVM 하이퍼바이저는 Red Hat Enterprise Linux(RHEL) 10.1, 10.0, 9.7 및 9.6과 Rocky Linux 10.0, 9.7 및 9.6에서 지원됩니다.
+
--
[NOTE]
====
...
NetApp 라이센스 파일 지원이 업데이트되었습니다:: ONTAP Select 9.16.1부터 NetApp 라이센스 파일(NLF) 지원이 업데이트되었습니다. 새로운 NLF 형식에는 ARP, ONTAP S3 및 S3 SnapMirror 기능에 대한 라이센스가 포함됩니다. link:reference_lic_ontap_features.html#licensed-ontap-features["자세한 정보"].
+
--
...
--
자율적 랜섬웨어 방어 지원:: ONTAP Select 9.16.1은 Autonomous Ransomware Protection(ARP)에 대한 지원을 도입합니다. ONTAP Select 9.16.1은 ARP에 대한 수동 업데이트만 지원하며 자동 업데이트는 지원되지 않습니다. ARP 기능 라이센스는 ONTAP Select 9.16.1용 NLF에 포함되어 있습니다. link:reference_lic_ontap_features.html#licensed-ontap-features["자세한 정보"].
...
NOTE: 클러스터 확장 및 축소 지원은 ESXi 클러스터로만 제한됩니다.
...
ONTAP Select 배포 지원이 업데이트되었습니다:: ONTAP Select Deploy 9.14.1 버전이 9.14.1P2보다 낮은 경우, 가능한 한 빨리 ONTAP Select Deploy 9.14.1P2로 업그레이드해야 합니다. 자세한 내용은 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 릴리스 정보"^]를 참조하십시오.
...
ONTAP Select 배포 지원이 업데이트되었습니다:: 2024년 4월 현재, ONTAP Select Deploy 9.13.1은 NetApp 지원 사이트에서 더 이상 제공되지 않습니다. ONTAP Select Deploy 9.13.1을 사용 중인 경우, 가능한 한 빨리 ONTAP Select Deploy 9.14.1P2로 업그레이드하십시오. 자세한 내용은 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 릴리스 정보"^]을 참조하십시오.
...
2024년 4월 현재, ONTAP Select Deploy 9.12.1은 NetApp 지원 사이트에서 더 이상 제공되지 않습니다. ONTAP Select Deploy 9.12.1을 사용 중인 경우, 가능한 한 빨리 ONTAP Select Deploy 9.14.1P2로 업그레이드하십시오. 자세한 내용은 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 릴리스 정보"^]을 참조하십시오.
...
+
NetApp은(는) 고객이 ONTAP Select for KVM에서 ONTAP Select for ESXi를 포함한 다른 모든 ONTAP 플랫폼으로 전체 데이터 마이그레이션을 계획하고 실행할 것을 강력히 권장합니다. 자세한 내용은  https://mysupport.netapp.com/info/communications/ECMLP2877451.html["EOA 통지"^]를 참조하십시오.
