...
. 페이지 오른쪽 상단의 사람 모양 아이콘을 선택한 다음 *비밀번호 변경*을 선택합니다.
. 안내에 따라 현재 비밀번호와 새 비밀번호를 입력하고 * 제출 * 을 선택합니다.
...
. *Management Servers* > *Add vCenter*를 선택합니다.
. 다음 정보를 입력하고 * 추가 * 를 선택합니다.
+
[cols="30,70"]
|===
| 이 필드에서… | 다음을 수행합니다. 
...
|===
. 필요에 따라 새 관리 서버가 추가된 후 image:icon_kebab.gif["옵션"]를 선택한 다음 다음 중 하나를 선택하십시오.
+
** 자격 증명을 업데이트합니다
** 자격 증명을 확인합니다
** 관리 서버를 제거합니다
