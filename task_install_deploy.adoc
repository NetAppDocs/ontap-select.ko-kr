...
NetApp 지원 사이트에서 ONTAP Select 패키지를 다운로드하십시오.
...
설치 패키지를 설치하기 전에 ONTAP Select Open Virtualization Appliance(OVA)의 서명을 확인하십시오.
...
OVF VM 이미지를 사용하여 ONTAP Select Deploy VM을 설치하고 시작하십시오. 설치 과정의 일부로 네트워크 인터페이스를 DHCP 또는 고정 IP 구성으로 설정합니다.
...
== 4단계: 배포 웹 인터페이스에 Sign in.

웹 사용자 인터페이스에 Sign in 배포 유틸리티를 사용할 수 있는지 확인하고 초기 구성을 수행하십시오.
