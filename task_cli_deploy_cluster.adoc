...
.하드웨어 RAID
--
로컬 하드웨어 RAID 컨트롤러를 사용하는 경우 시스템 데이터와 루트 및 데이터 애그리게이트를 위해 각 노드에 최소 하나의 데이터 저장소(ESXi) 또는 하나의 link:kvm-host-configuration-and-preparation-checklist.html#step-2-create-a-storage-pool["스토리지 풀(KVM)"]를 생성해야 합니다. ONTAP Select 노드 구성 시 스토리지 풀을 연결해야 합니다.

--
.소프트웨어 RAID
--
소프트웨어 RAID를 사용하는 경우 시스템 데이터용 데이터스토어(ESXi) 또는 link:kvm-host-configuration-and-preparation-checklist.html#step-2-create-a-storage-pool["스토리지 풀(KVM)"]를 하나 이상 생성하고 루트 및 데이터 애그리게이트에 SSD 드라이브를 사용할 수 있는지 확인해야 합니다. ONTAP Select 노드를 구성하는 과정에서 스토리지 풀과 디스크를 연결해야 합니다.
