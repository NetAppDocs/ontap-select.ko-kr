...
* 소프트웨어 RAID를 사용하는 ONTAP Select(KVM) 및 ONTAP Select(ESX)의 경우, 소프트웨어 RAID를 지원하지 않는 이전 버전으로 되돌릴 수 없습니다. 또한, ESXi에 ONTAP Select 9.5 이상 버전을 새로 설치하면 VMXNET3 네트워크 드라이버와 (가능한 경우) vNMVE 드라이버가 사용됩니다. 이러한 새 설치는 이전 버전의 ONTAP Select로 되돌릴 수 없습니다.
...
Deploy 인스턴스가 특정 환경에서 실행된 후에는 해당 인스턴스의 네트워킹 세부 정보를 수정할 수 없습니다. 자세한 내용은 기술 자료 문서를 참조하십시오. link:https://kb.netapp.com/onprem/ontap/ONTAP_Select/Modifying_DNS_configuration_of_ONTAP_Deploy_instance["ONTAP Deploy 인스턴스의 DNS 구성 수정"^] .
...
예를 들어 ONTAP Select VM이나 호스트가 완전히 손실되거나 단일 노드 클러스터가 실수로 삭제된 경우 ONTAP Select 노드를 복구해야 하는 경우 NetApp 기술 지원팀에 문의하세요. 자세한 내용은 기술 자료 문서를 참조하십시오. link:https://kb.netapp.com/on-prem/ontap/ontap-select/Select-KBs/ONTAP_Select_node_VM_recovery_in_case_of_disaster["재해 발생 시 ONTAP Select 노드 VM 복구"^] .
