...
NOTE: ONTAP Select 9.10.1부터는 KVM 하이퍼바이저에 새 클러스터를 더 이상 구축할 수 없습니다. ONTAP Select 9.11.1 이상 버전에서는 오프라인 및 삭제 기능을 제외한 모든 관리 기능을 기존 KVM 클러스터 및 호스트에서 사용할 수 없습니다. KVM과 관련된 질문은 KVM을 지원하는 이전 릴리스에만 해당됩니다.
