...
. *설정 및 AutoSupport*을 선택한 다음 image:icon_pencil.gif["편집"]을 선택합니다.
. 사용 환경에 맞는 구성 데이터를 입력하고 *수정*을 선택하십시오.
+
...
. 관리자 계정을 사용하여 배포 유틸리티 웹 사용자 인터페이스에 로그인합니다.
. 페이지 맨 위에서 * 관리 * 탭을 선택합니다.
. *이벤트 및 작업*을 선택한 다음 *이벤트*를 선택합니다.
. 선택적으로 * 필터 * 를 선택하여 표시되는 이벤트 메시지를 제한하는 필터를 만들 수 있습니다.
...
. 관리자 계정을 사용하여 배포 유틸리티 웹 사용자 인터페이스에 로그인합니다.
. 페이지 맨 위에서 * 관리 * 탭을 선택합니다.
. *설정 및 AutoSupport*을 선택한 다음 image:icon_kebab.gif["옵션"]을 선택합니다.
...
. 필요에 따라 *AutoSupport 기록*에서 올바른 패키지를 선택한 다음 다운로드 아이콘을 선택하여 AutoSupport 파일을 로컬 워크스테이션에 저장할 수 있습니다.
